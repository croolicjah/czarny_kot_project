...
class Login(View):
    ...
    def post(self, request, *args, **kwargs):
        user = auth.authenticate(request, username=request.POST['user'], password=request.POST['password'])
        if user is not None:
            auth.login(request, user)
            return redirect('home')
        else:
            return render(
                ...
                {'error': 'Niewłaściwe hasło lub nazwa użytkownika. Spróbuj jeszcze raz.'}
            )
